    ...
    async (error, user) => {
      ...
      if (user) {
        ...
        if (verPass) {
          ...
          const token = jwt.sign({_id:activeUser._id,username:activeUser.username,deposit:activeUser.deposit,productsPurchased:activeUser.productsPurchased,connected:activeUser.connected,role:activeUser.role}, 
            "MERNSECRET", {
            expiresIn: "5h" 
          });
          res.status(200).json({
        ...
        return res.status(405).json({ message: "Something went wrong when trying to LogIn" });
      }
    }
  );
  //Promise
    ...
    async (error, user) => {
      ...
      if (user) {
        ...
        if (verPass) {
         ...
         return res.status(200).json({
          message:"Log out from all the sessions Suceesfully ! , You can log in now ",
          data:activeUser
         })
        } else {
          return res.status(400).json({
        ...
        return res.status(400).json({ message: "Something went wrong when trying to Log out all " });
      }
    }
  );
  //Promise
